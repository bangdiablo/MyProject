<template>
    <div class="policy_write_form">
                ...
                <tr v-for="(row, index) in rows">
                    <td>
                        <button type="button" class="btn_variable"
                                @click="coldFilePopup(row.bf_filepath, index, 'file')">
        ...
        <div class="pwf_fieldset">
            ...
            <table>
                ...
                <tr v-for="(row, index) in rowsfolder">
                    <td>
                        <button type="button" class="btn_variable"
                                @click="insertFolder(row.bf_filepath, index, 'folder')">{{
                            $t('policy.general.insert_var') }}
                        </button>
                    </td>
        ...
        <!-- File/Folder POPUP -->
        <div class="layer_popup preview" v-show="cold_show">
            <h3>변수 삽입</h3>
            ...
            <div class="btn_set">
                ...
                <button type="button" class="btn_basic cancel" @click="coldFilePopup">닫기</button>
            </div>
            <button class="btn_close" @click="coldFilePopup">
                <img src="@/assets/images/component/btn_popup_close.png" alt="닫기"/>
            </button>
        </div>
        <!-- END File/Folder POPUP -->
        <!-- Filter POPUP -->
        <div class="layer_popup preview" v-show="filter_show">
            <h3>필터 추가</h3>
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            filter_show() {
                //filter popup update
                if (this.setPolicyData.policycoldfolder[this.index_div].bf_extensionchk == "Y") {
                    this.bf_extensionchk = true
                } else {
                    this.bf_extensionchk = false
                }
                this.bf_filetypechk = this.setPolicyData.policycoldfolder[this.index_div].bf_filetypechk

                if (this.setPolicyData.policycoldfolder[this.index_div].bf_filetypechk == 'Y') {
                    if (this.setPolicyData.policycoldfolder[this.index_div].bf_regexchk == "Y") {
                        this.efIncludechk = true;
                    } else {
                        this.efIncludechk = false;
                    }
                    this.efInclude = this.setPolicyData.policycoldfolder[this.index_div].bf_expression
                } else {
                    if (this.setPolicyData.policycoldfolder[this.index_div].bf_regexchk == "Y") {
                    ...
                    this.efExclude = this.setPolicyData.policycoldfolder[this.index_div].bf_expression
                }
                if (this.setPolicyData.policycoldfolder[this.index_div].bf_filebackchk == "Y") {
                    this.bf_filebackchk = true
                } else {
                    this.bf_filebackchk = false
                }
                this.bf_size = this.setPolicyData.policycoldfolder[this.index_div].bf_size
                this.bf_volumechk = this.setPolicyData.policycoldfolder[this.index_div].bf_volumechk
                if (this.setPolicyData.policycoldfolder[this.index_div].bf_datebackchk == "Y") {
                    this.bf_datebackchk = true;
                } else {
                    this.bf_datebackchk = false;
                }
                this.bf_dateback = this.setPolicyData.policycoldfolder[this.index_div].bf_dateback
            }
        },
        methods: {
            select_all() {
            ...
            addRemoveRow(index) {
                ...
                this.setPolicyData.policycoldfiles.splice(index, 1)
            },
            addremoveFolder(index) {
                this.rowsfolder.splice(index, 1)
                this.setPolicyData.policycoldfolder.splice(index, 1)
            },
            coldFilePopup(filepath, index, div) {
                //POPUP
                this.index_div = index
                this.vcheck = filepath
                this.insert_div = div
                this.cold_show = !this.cold_show;
            },
            ...
            sendVariable() {
                ...
                this.setPolicyData.policycoldfiles.push(fileData)
                this.cold_show = !this.cold_show;
            },
            sendFolderVariable() {
                ...
                this.setPolicyData.policycoldfolder.push(fileData)
                this.cold_show = !this.cold_show;
            },
            onKeyup(event) {
                //숫자만입력
            ...
            save() {
                //Filter data update
                this.setPolicyData.policycoldfolder[this.index_div].bf_extensionchk = this.bf_extensionchk ? 'Y' : 'N'
                if (this.bf_filetypechk == 'Y') {
                    this.setPolicyData.policycoldfolder[this.index_div].bf_regexchk = this.efIncludechk ? 'Y' : 'N'
                    this.setPolicyData.policycoldfolder[this.index_div].bf_expression = this.efInclude
                } else {
                    this.setPolicyData.policycoldfolder[this.index_div].bf_regexchk = this.efExcludechk ? 'Y' : 'N'
                    this.setPolicyData.policycoldfolder[this.index_div].bf_expression = this.efExclude
                }
                this.setPolicyData.policycoldfolder[this.index_div].bf_filebackchk = this.bf_filebackchk ? 'Y' : 'N'
                this.setPolicyData.policycoldfolder[this.index_div].bf_size = this.bf_size
                this.setPolicyData.policycoldfolder[this.index_div].bf_volumechk = this.bf_volumechk
                this.setPolicyData.policycoldfolder[this.index_div].bf_datebackchk = this.bf_datebackchk ? 'Y' : 'N'
                this.setPolicyData.policycoldfolder[this.index_div].bf_dateback =
        ...
        created() {
            ...
            this.setPolicyData.policycoldfolder = [];
